...
ifeq ($(REGISTRY_ENABLED),true)
ifeq ($(TOOL),oc)
	$(eval PLUGIN_REGISTRY_URL := $(shell echo http://$$($(TOOL) get route che-plugin-registry -n $(NAMESPACE) -o jsonpath='{.spec.host}')))
else
	$(eval PLUGIN_REGISTRY_URL := $(shell echo http://$$($(TOOL) get ingress che-plugin-registry -n $(NAMESPACE) -o jsonpath='{.spec.rules[0].host}')))
endif
else
	PLUGIN_REGISTRY_URL=""
endif
...
_update_yamls: _set_registry_url
	sed -i.bak -e "s|plugin.registry.url: .*|plugin.registry.url: $(PLUGIN_REGISTRY_URL)|g" ./deploy/controller_config.yaml
...
_reset_yamls: _set_registry_url
	sed -i.bak -e "s|plugin.registry.url: .*|plugin.registry.url: http://che-plugin-registry.192.168.99.100.nip.io/v3|g" ./deploy/controller_config.yaml
