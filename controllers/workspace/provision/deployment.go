...
// SetDeploymentSpecStartedToFalse sets deployment.spec.started to false
func SetDeploymentSpecStartedToFalse(workspace *dw.DevWorkspace, client runtimeClient.Client) error {
	...
	for _, pod := range podList.Items {
		for _, containerStatus := range pod.Status.ContainerStatuses {
			if containerStatus.State.Waiting != nil {
				if containerStatus.State.Waiting.Reason == ContainerCrashLoopBackOffReason || containerStatus.State.Waiting.Reason == ContainerImagePullErr {
