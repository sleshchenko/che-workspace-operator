...
func deleteJob(client crclient.Client, job *batchv1.Job) error {
	...
	pods, err := GetPodsBySelector(client, job.Namespace, "job-name="+job.Name)
	if err != nil {
		return err
	}
	for _, pod := range pods.Items {
		log.Info(fmt.Sprintf("WARN: Job '%s/%s' related pod %s is not clean automatically", job.Namespace, job.Name, pod.Name))
		err = DeletePod(client, job.Namespace, pod.Name)
		if err != nil {
			return err
		}
	}
