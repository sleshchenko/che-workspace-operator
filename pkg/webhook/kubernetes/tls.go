...
func SetupSecureService(client crclient.Client, ctx context.Context, namespace string) error {
	...
	err := client.Get(context.TODO(), types.NamespacedName{Namespace: namespace, Name: TLSSelfSignedCertificateSecretName}, devworkspaceSecret)
	if err == nil {
		return nil
	}
	...
	err = webhook.CreateSecureService(client, ctx, namespace, map[string]string{})
	if err != nil {
	...
	err = tls.GenerateCerts(client, ctx, tls.GenCertParams{
		...
		Namespace:     namespace,
		//TODO it's not really clean which secret is supposed to hold what. Maybe even I assigned params wrong
